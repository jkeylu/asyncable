...
(function() {
  ...
  var _reversedParamsAsyncIteratorConvertor = _compose(_reverseTwoParams, _convertToAsyncIterator);
  ...
  asyncable.auto = _convertToAsyncable('auto', function(args) {
    ...
    tasks = _isArray(tasks)
      ? _map(tasks, _reversedParamsAsyncIteratorConvertor)
      : _mapKeys(tasks, function(task, name) {
        var arr;
        if (_isArray(task)) {
          arr = task;
          task = arr.pop();
          arr.push(_reversedParamsAsyncIteratorConvertor(task));
          return arr;
        } else {
          return _reversedParamsAsyncIteratorConvertor(task);
        }
      });
  ...
  asyncable.waterfall = _convertToAsyncable('waterfall', function(args) {
    var tasks = args.shift();
    if (!_isArray(tasks)) {
      throw new Error('First argument to waterfall must be an array of functions');
    }
    tasks = _map(tasks, function(task) {
      return function() {
        var args = _slice.call(arguments);
        var callback = args.pop();
        ...
        var promise;
        try {
          promise = task.apply(this, args);
        } catch(e) {
          return callback(e);
        }

        if (promise && _isPromise(promise)) {
          promise.then(function(results) { callback.apply(this, [null].concat(results)); }, callback);
        } else {
          callback.apply(this, [null].concat(promise));
        }
      };
    });
    args.unshift(tasks);
  ...
  _arrayEach(['applyEach', 'applyEachSeries'], function(fnName) {
    asyncable[fnName] = function(/* fns, args... */) {
      var args = _slice.call(arguments);
      if (args.length === 0) {
        return Promise.reject(new Error(fnName + ' at least one argument needed'));
      }
  ...
  asyncable.retry = _convertToAsyncable('retry', function(args) {
    var iterator = args.pop();
    return args.concat(_reversedParamsAsyncIteratorConvertor(iterator));
  });
  ...
  asyncable.delay = function(delay) {
    return function() {
      return new Promise(function(resolve, reject) {
  ...
  asyncable.thenable = function(fn) {
    return function() {
      var args = _slice.call(arguments);
      return new Promise(function(resolve, reject) {
